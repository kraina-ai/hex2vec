...
class Combined(pl.LightningModule):
    def __init__(self, encoder_sizes):
        ...
        decoder_sizes = [(output_size, input_size) for input_size, output_size in sizes][::-1]
    ...
    def training_step(self, batch, batch_idx):
        ...
        prediction_loss = (F.binary_cross_entropy_with_logits(scores_pos, y_pos) + F.binary_cross_entropy_with_logits(scores_neg, y_neg)) / 2
        reconstruction_loss = (F.mse_loss(Xt, Xt_hat) + F.mse_loss(Xc, Xc_hat) + F.mse_loss(Xn, Xn_hat)) / 3
        ...
        self.log('train_reconstruction_loss', reconstruction_loss, on_step=True, on_epoch=True)
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_reconstruction_loss', reconstruction_loss, on_step=True, on_epoch=True)
        return loss
